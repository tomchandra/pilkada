...
use App\Libraries\Systems\Secured;
use App\Libraries\Systems\AuthAccess;
use App\Libraries\Systems\Breadcrumbs;
...
if (!function_exists('auth_access')) {
   function auth_access()
   {
      ...
      return $access->getAuthAccessMenuByUser();
   }
}

if (!function_exists('groups_menus')) {

   function groups_menus()
   {
      $access = new AuthAccess();
      return $access->getAuthAccessMenuByGroup();
   }
}

if (!function_exists('auth_buttons')) {
   function auth_buttons()
   {
      $access = new AuthAccess();
      return $access->getAuthAccessButtons();
   }
}
...
if (!function_exists('breadcrumb')) {
   function breadcrumb()
   {
      ...
      // $breadcrumbs->add('Customer', '/dashboard/customer');
...
if (!function_exists('currentPage')) {
   function currentPage()
   {
      ...
      $controller     = is_array($controllerName) ? strtolower(end($controllerName)) : "";
      $modules        = is_array($controllerName) ? strtolower(prev($controllerName)) : "";

      if (!empty($controller) && !empty($modules)) {
         session()->set('current_page', join("/", [site_url(), $modules, $controller]));

         $key = join("_", ["menu_id", $modules, $controller]);
         if (session()->has($key)) {
            session()->set('active_menu_id', session()->get($key));
         }
      }

      return [$modules, $controller];
   }
}
...
if (!function_exists('render_tree')) {
   ...
   function render_trees($data, $root = 0)
   {
      if (is_array($data) && count($data) > 0) {
         ...
         $trees = [];
         foreach ($data as $key => $value) {
            if (in_array($value['parent_id'], $id) || $value['parent_id'] == 0) {
               $trees[] = $value;
            }
         }


         return create_tree($trees, $root);
      }
...
if (!function_exists('create_tree_branch')) {
   function create_tree_branch(&$parents, $children)
   {
      $tree = array();
      foreach ($children as $child) {
         if (isset($parents[$child['id']])) {
            $child['children'] = create_tree_branch($parents, $parents[$child['id']]);
         }
         $tree[] = $child;
      }
      return $tree;
...
if (!function_exists('create_tree')) {
   function create_tree($flat, $root = 0)
   {
      if (count($flat) < 1) {
         return [];
      }
      ...
      return create_tree_branch($parents, $parents[$root]);
   }
}
